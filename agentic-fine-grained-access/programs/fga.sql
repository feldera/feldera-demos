...
-- A log of what a user has accessed over time.
create table access_log (
    user_id bigint not null, -- user
    file_id bigint not null, -- what file
    access_request string not null, -- read or write
    ts timestamp not null, -- what time the file was requested
    ip string not null, -- ip of user
    browser string not null, -- browser of user
    device string not null -- what device
) with (
  'materialized' = 'true',
  'connectors' = '[{
    "name": "access_log",
        ...
        "plan": [
          {
            "limit": 1000000,
            "fields": {
                "user_id": {"range": [0, 1000]},
                "file_id": {"range": [1100, 101100], "strategy": "uniform" },
                "access_request": { "values": ["read", "write"] },
                "ts": { "range": ["2026-01-01T00:00:00Z", "2027-01-01T00:00:00Z"], "scale": 1000 },
                "ip": { "strategy": "ipv4" },
                "browser": { "strategy": "user_agent" },
                "device": { "values": ["iphone11", "iphone17", "ipad", "windows-pc", "macbook", "android11", "android15"] }
            }
          },
          {
            "rate": 1000,
            ...
            "fields": {
                ...
                "file_id": {"range": [1100, 101100], "strategy": "uniform"},
                "access_request": { "values": ["read", "write"] },
                "ts": { "range": ["2026-01-06T10:00:00Z", "2026-01-06T11:00:00Z"], "scale": 1 },
                "ip": { "values": ["192.168.99.1"] },
                "browser": { "values": ["python-requests/2.28.0"] },
                "device": { "values": ["unknown"] }
            }
          },
          {
            "rate": 1000,
            ...
            "fields": {
                "user_id": {"values": [99]},
                "file_id": {"range": [1100, 101100], "strategy": "uniform"},
                "access_request": { "values": ["read"] },
                "ts": { "range": ["2026-01-07T14:00:00Z", "2026-01-07T15:00:00Z"], "scale": 1 },
                "ip": { "values": ["10.0.0.55"] },
                "browser": { "values": ["curl/7.88.1"] },
                "device": { "values": ["unknown"] }
            }
          },
          {
            ...
            "fields": {
                "user_id": {"values": [256]},
                "file_id": {"range": [5000, 5050], "strategy": "uniform"},
                "access_request": { "values": ["read", "write"] },
                "ts": { "range": ["2026-01-08T09:00:00Z", "2026-01-08T09:30:00Z"], "scale": 1 },
                "ip": { "values": ["172.16.0.8"] },
                "browser": { "values": ["python-requests/2.31.0"] },
                "device": { "values": ["unknown"] }
            }
          }
        ]
