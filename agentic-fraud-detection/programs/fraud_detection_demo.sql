...
CREATE TABLE CUSTOMER (
    ...
    -- (see https://docs.feldera.com/connectors/sources/datagen)
    'connectors' = '[{
      "name": "customer",
      "transport": {
        "name": "datagen",
      ...
      "name": "datagen",
        "config": {
          "plan": [{
            ...
            "rate": 1000,
            "fields": {
              "category": { "values": ["gift card", "grocery", "travel", "games"] },
...
-- 2. The number of transactions whose shipping address is more than ~55 km away from
--    the card holder's home address (ST_DISTANCE returns degrees; 0.5° ≈ 55 km).
CREATE VIEW TRANSACTION_WITH_AGGREGATES AS
SELECT
   *,
   AVG(amt) OVER window_1_day as avg_1day,
   SUM(case when distance > 0.5 then 1 else 0 end) OVER window_1_day as count_1day
FROM
