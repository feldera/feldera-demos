...
from constants import (
    ...
    SIM_NAMES, ENGINE_INDICES, DEFAULT_MODE,
...
def load_pregenerated(data_dir, max_steps=None):
    """Load preload + batches from a directory produced by run_csv_pipeline.py.

    Expects:
      <data_dir>/transactions.csv   — preload history
      <data_dir>/batches/batch_NNNN.csv — one file per streaming batch

    Returns the same (preload, batches) structure as split_csv().
    """
    data_dir    = Path(data_dir)
    preload_csv = data_dir / "transactions.csv"
    ...
    # Scan preload: count rows and grab ts bounds from first/last data line.
    preload = {"path": preload_csv, "n_rows": 0, "ts_min": None, "ts_max": None}
    with open(preload_csv, newline="") as f:
        for row in csv_mod.DictReader(f):
            _upd(preload, row["ts"])

    # Scan each batch file (small — typically a few thousand rows).
    batches = []
    for path in batch_files:
        b = {"path": path, "n_rows": 0, "ts_min": None, "ts_max": None}
        with open(path, newline="") as f:
            for row in csv_mod.DictReader(f):
                _upd(b, row["ts"])
        batches.append(b)

    print(f"[pregenerated] preload={preload['n_rows']:,} rows  "
          f"batches={len(batches)} × {batches[0]['n_rows'] if batches else 0} rows")
    return preload, batches


def _load_split(args):
    """Return (preload, batches): use pre-generated batches/ dir if present, else split_csv."""
    data_dir = Path(args.data_dir)
    if (data_dir / "batches").is_dir():
        return load_pregenerated(data_dir, max_steps=N_STEPS if N_STEPS > 0 else None)
    return split_csv(args.data_dir, args.steps, PRELOAD_ROWS, args.batch_rows)
...
def _build_coordinator(args, active_sims, skip_clickhouse, skip_feldera, api_url, api_key, start_event):
    ...
    from engine_postgres import PostgresFullEngine
    ...
    if not skip_postgres:
        engines.append(PostgresFullEngine(
            args.postgres_host, args.postgres_port, args.postgres_database,
            args.postgres_user, args.postgres_password))

    # Split CSV and load each storage group in parallel (primary before secondary).
    preload, batches = _load_split(args)
    batches = batches[:N_STEPS]   # honour --max-steps (N_STEPS = min(--max-steps, --steps))
    N_STEPS = len(batches)        # clamp to actual batch count (pre-generated dirs may have fewer)
...
def _run_sequential_benchmark(args, active_sims, skip_clickhouse, skip_feldera,
                               skip_postgres, api_url, api_key) -> tuple:
    ...
    for storage_key, group in storage_groups.items():
        ...
        pg_cap   = getattr(args, "postgres_steps", None)
        n_engine_steps = (pg_cap if (pg_cap and primary.sim_id == 2) else len(batches))
        print(f"[{primary.name}] Load complete ({elapsed:.1f}s). Running {n_engine_steps} steps …\n")

        # ── All steps for this engine group ───────────────────────────────
        seen_cc_nums: dict[int, set] = {e.sim_id: set() for e in group}
        for step_idx in range(n_engine_steps):
            rows = _parse_std_rows(batches[step_idx]["path"])
            ws   = batch_starts[step_idx]
            we   = win_ends[step_idx]
        ...
        # ── Teardown: free server-side resources before the next engine runs ──
        for e in group:
            e.teardown()
...
def main():
    ...
    parser.add_argument("--output",       default=None, metavar="FILE",
                        help="Save summary table here (default: results/<data_dir>_<timestamp>.txt)")
    parser.add_argument("--mock",         action="store_true",
                        help="Simulate queries (no DB needed)")
    parser.add_argument("--mode",         nargs="+", default=DEFAULT_MODE,
                        choices=list(ENGINE_INDICES),
                        metavar="ENGINE",
    ...
    parser.add_argument("--no-postgres",   action="store_true",
                        help="Disable PostgreSQL engine")
    ...
    parser.add_argument("--postgres-host",       default=POSTGRES_HOST)
    parser.add_argument("--postgres-port",       type=int, default=POSTGRES_PORT)
    parser.add_argument("--postgres-database",   default=POSTGRES_DATABASE)
    parser.add_argument("--postgres-user",       default=POSTGRES_USERNAME)
    parser.add_argument("--postgres-password",   default=POSTGRES_PASSWORD)
    ...
    parser.add_argument("--postgres-steps", type=int, default=None,
                        help="Limit PostgreSQL to this many steps (default: same as --steps). "
                             "Useful when PG is very slow and you only need a few data points.")
    ...
    if args.output is None:
        ...
        data_tag = Path(args.data_dir).name
        args.output = f"results/{data_tag}_{ts}.txt"

    from engine_feldera import DEFAULT_API_URL, DEFAULT_API_KEY

    # ── Connectivity pre-check ─────────────────────────────────────────────
    if not args.mock:
        _active = [ENGINE_INDICES[e] for e in args.mode]
        _need_clickhouse = 0 in _active and not args.no_clickhouse
        _need_feldera    = 1 in _active and not args.no_feldera
        errors = []
        if _need_clickhouse:
            try:
        ...
        _need_postgres = 2 in _active and not args.no_postgres
        if _need_postgres:
            try:
                import psycopg2 as _pg
                _c2 = _pg.connect(host=args.postgres_host, port=args.postgres_port,
                                  dbname="postgres", user=args.postgres_user,
                                  password=args.postgres_password, connect_timeout=3)
                _c2.close()
            except Exception:
                errors.append(
                    f"PostgreSQL unreachable at {args.postgres_host}:{args.postgres_port}\n"
                    f"  Start it with:  docker start postgres\n"
                    f"  Or:  docker run -d --name postgres "
                    f"-e POSTGRES_PASSWORD='' -e POSTGRES_HOST_AUTH_METHOD=trust "
                    f"-p 5432:5432 postgres"
                )
        if _need_feldera:
    ...
    active_sims = [ENGINE_INDICES[e] for e in args.mode]
    ...
    skip_postgres   = 2 not in active_sims
    ...
    if args.mock:
        ...
        perf_data, preload_times, split_meta = _run_sequential_benchmark(
            args, active_sims, skip_clickhouse, skip_feldera, skip_postgres, api_url, api_key)
    ...
    print(f"\nDemo [{' '.join(args.mode)}]: {sim_names_str}")
