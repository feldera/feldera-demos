...
try:
    ...
    _env = dotenv_values(Path(__file__).parents[3] / ".env")
except ImportError:
    _env = {}
...
class _FelderaEngine:
    ...
    def setup(self, sql: str, runtime_config: RuntimeConfig | None = None,
              max_rss_mb: int | None = None) -> None:
        resources = Resources(memory_mb_max=max_rss_mb) if max_rss_mb is not None else Resources()
        cfg = runtime_config or RuntimeConfig(workers=16, resources=resources)
        self._pipeline = (
...
def setup_and_start(api_url: str, api_key,
                    ...
                    prio: dict, max_rss_mb: int | None = None) -> _FelderaEngine:
    ...
    if max_rss_mb is not None:
        print(f"[feldera] max_rss_mb={max_rss_mb:,}")
    engine = _FelderaEngine(api_url, api_key)
    engine.setup(sql, max_rss_mb=max_rss_mb)
...
class FelderaFraudEngine(FraudEngine):
    ...
    def teardown(self) -> None:
        if self._engine is None or self._engine._pipeline is None:
            return
        try:
            self._engine._pipeline.stop()
            self._engine._pipeline.delete()
            print("[feldera] Pipeline stopped and deleted.")
        except Exception as e:
            print(f"[feldera] teardown warning: {e}")
        self._engine._pipeline = None
