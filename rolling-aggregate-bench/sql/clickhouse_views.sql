...
-- │  flagged_gb30 + flagged_gb45 +           │  (inlined in fraud_alerts UNION)  │
-- │    flagged_sv7 + flagged_disp            │                                   │
...
TRANSACTION_WITH_AGGREGATES AS (
    ...
    WINDOW
        window_3day  AS (PARTITION BY cc_num ORDER BY toUnixTimestamp64Second(ts) RANGE BETWEEN {window_3d_secs}  PRECEDING AND CURRENT ROW),  -- 3 days
        window_7day  AS (PARTITION BY cc_num ORDER BY toUnixTimestamp64Second(ts) RANGE BETWEEN {window_7d_secs}  PRECEDING AND CURRENT ROW),  -- 7 days
        window_30day AS (PARTITION BY cc_num ORDER BY toUnixTimestamp64Second(ts) RANGE BETWEEN {window_30d_secs} PRECEDING AND CURRENT ROW), -- 30 days
        window_45day AS (PARTITION BY cc_num ORDER BY toUnixTimestamp64Second(ts) RANGE BETWEEN {window_45d_secs} PRECEDING AND CURRENT ROW)  -- 45 days
),
flagged_gb30 AS (
